...
export default function ArProductsPage() {
  ...
  function openEdit(product: ArProduct) {
    ...
    setForm({
      ...
      isRequired: Boolean(product.isRequired),
    });
    setFormError(null);
    setModalOpen(true);
  }
  ...
  const columns = useMemo<Column<ArProduct>[]>(
    () => [
      ...
      },
      {
        id: "required",
        header: "Required",
        accessor: (r) => (r.isRequired ? "yes" : "no"),
        cell: (r) =>
          r.isRequired ? (
            <Chip tone="positive">Required on invoices</Chip>
          ) : (
            <Chip>Optional</Chip>
          ),
      },
      ...
      >
        <div className="space-y-4">
          ...
          <label className="flex items-start gap-2 text-sm text-slate-700">
            ...
            />
            <span>
              <span className="font-medium">Required on new invoices</span>
              <span className="mt-0.5 block text-xs text-slate-500">
                Auto-adds this product as a line item when creating an invoice.
                Optional products stay available in the add-product dropdown.
              </span>
            </span>
          </label>
