...
/**
 * Partner (payable) reports — same engines, payer-facing copy.
 * Partners only pay invoices; they have nothing to collect.
 */
export const PARTNER_REPORTS: ReportDefinition[] = [
  {
    id: "invoice_summary",
    label: "Invoices owed",
    description: "Each bill sent to you with amount due and what you’ve paid",
    icon: FileText,
    accent: "indigo",
    serverType: "invoice_register",
  },
  {
    id: "receivable_aging",
    label: "Payable aging",
    description: "What you still owe, grouped by how far past due",
    icon: Layers,
    accent: "orange",
    serverType: "invoice_aging",
  },
  {
    id: "payment_history",
    label: "Payments made",
    description: "Your payments with method and transaction ID",
    icon: Wallet,
    accent: "teal",
    serverType: "payment_register",
  },
  {
    id: "revenue",
    label: "Billed vs paid",
    description: "Amounts billed to you by month compared with what you paid",
    icon: TrendingUp,
    accent: "blue",
    serverType: "monthly_revenue",
  },
  {
    ...
    description: "How much you paid each month",
    icon: CircleDollarSign,
    accent: "green",
    serverType: "monthly_collections",
  },
  {
    ...
    description: "Bills past due with days late and any late fees",
    icon: AlarmClock,
    accent: "red",
    serverType: "late_fees",
  },
...
export function reportsForRole(role: string): ReportDefinition[] {
  return role === "partner" ? PARTNER_REPORTS : REPORTS;
}

/** Rewrite collector language into payer (payable) language for partners. */
export function asPayableReport(result: ReportResult): ReportResult {
  const mapLabel = (text: string) => {
    const pairs: [string, string][] = [
      ["Total invoiced", "Total billed to you"],
      ["Total collected", "Total you paid"],
      ["Invoiced value", "Billed to you"],
      ["Balance due", "Still to pay"],
      ["Total outstanding", "Still to pay"],
      ["Total received", "Total you paid"],
      ["Collection rate", "Share paid"],
      ["Cash collected by month", "Payments by month"],
      ["Invoiced vs collected by month", "Billed vs paid by month"],
      ["Balance by aging bucket", "What you owe by age"],
      ["Receipts by month", "Your payments by month"],
      ["Outstanding balance by customer", "Your open balance"],
      ["Invoiced", "Billed"],
      ["Collected", "Paid"],
      ["Received", "Paid"],
      ["Average days to pay", "Average days to settle"],
      ["Strongest month", "Highest billed month"],
      ["Late balance by aging bucket", "Overdue amount by age"],
      ["Amount late", "Amount overdue"],
      ["Late invoices", "Overdue invoices"],
    ];
    let out = text;
  ...
  return {
    ...
    summary: result.summary.map((s) => ({
      ...s,
      label: mapLabel(s.label),
      hint: s.hint ? mapLabel(s.hint) : s.hint,
    })),
    chartTitle: mapLabel(result.chartTitle),
    chart: result.chart
      ? result.chart.type === "area" || result.chart.type === "bar"
        ? {
            ...result.chart,
            series: result.chart.series.map((s) => ({
              ...s,
              label: mapLabel(s.label),
            })),
          }
    ...
    columns: result.columns.map((c) => ({
      ...c,
      header: mapLabel(c.header)
        .replace(/^Collected$/, "Paid")
        .replace(/^Invoiced$/, "Billed")
        .replace(/Collection rate/i, "Share paid"),
    })),
  };
}
